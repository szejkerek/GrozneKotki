...
namespace BetterFolders
{
    ...
    public static class FolderColors
    {
        ...
        static FolderColors()
        {
            EditorApplication.delayCall += () =>
            {
                ...
                // Jesli nie ma ustawien, po prostu nie podpinamy handlera
                if (settings != null)
                {
  ...
  // At class scope
private const string FolderColorSettingsGuid = "6fb55cfc97823b344acd52701a37e6d0";
private const string SettingsSearchFolder = "Assets/Settings";

private static void LoadSettings()
{
    settings = null;
    string settingsAssetPath = string.Empty;

    // 1) Try by fixed guid
    if (!string.IsNullOrEmpty(FolderColorSettingsGuid))
    {
        settingsAssetPath = AssetDatabase.GUIDToAssetPath(FolderColorSettingsGuid);
        if (!string.IsNullOrEmpty(settingsAssetPath))
        {
            settings = AssetDatabase.LoadAssetAtPath<FolderColorSettings>(settingsAssetPath);
        }
    }

    // 2) Fallback search in Assets/Settings
    if (settings == null)
    {
        string[] guids = AssetDatabase.FindAssets("t:FolderColorSettings", new[] { SettingsSearchFolder });
        if (guids != null && guids.Length > 0)
        {
            settingsAssetPath = AssetDatabase.GUIDToAssetPath(guids[0]);
            settings = AssetDatabase.LoadAssetAtPath<FolderColorSettings>(settingsAssetPath);
        }
    }

    // 3) If still null, do not create a new asset, just warn and exit
    if (settings == null)
    {
        Debug.LogWarning(
            $"FolderColorSettings asset not found. Expected it in {SettingsSearchFolder} " +
            $"or at guid {FolderColorSettingsGuid}. Please create it or move it there."
        );
        return;
    }

    // 4) Ensure folderRules list exists
    ...
    // 5) Optional default preset import if rules are empty
    if (settings.folderRules.Count == 0)
    {
        EditorApplication.delayCall += () =>
        {
            ...
            if (defaultPreset != null)
            {
                try
                {
                    var wrapper = JsonUtility.FromJson<FolderColorSettingsEditor.PresetWrapper>(defaultPreset.text);
                    var importedRules = wrapper != null ? wrapper.folderRules : null;
            ...
            {
                Debug.LogWarning(
                    $"Could not find any preset. Searched paths:\n{string.Join("\n", possiblePaths)}"
                );
                settings.folderRules = new List<FolderRule>();
            }
...
{
    // Gdy nie ma ustawien, nic nie robimy
    if (settings == null)
        return;
